...
export default function PostList({ postList }: PostListProps): JSX.Element {
    ...
    const getMorePost = async () => {
        const response = await fetch(`https://jsonplaceholder.typicode.com/posts?_start=${posts.length}&_limit=9`)
        const newPosts = await response.json()

        newPosts.length !== 0 ? setPosts((post) => [...post, ...newPosts]) : setHasMore(false)
    }
